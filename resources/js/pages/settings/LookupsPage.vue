        ...
        <div class="space-y-6">
            ...
            <section v-else-if="lookup === 'user-roles'" class="space-y-6">
                <p class="text-sm text-slate-600">
                    User roles can be viewed here. Create new role assignments and update assignment status from User Account Management.
                </p>
                <div class="rbim-card overflow-hidden">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr v-for="role in roleCatalog" :key="role.role_name">
                                <td class="px-4 py-3 font-medium text-slate-900">{{ role.role_name }}</td>
                                <td class="px-4 py-3 text-slate-600">{{ role.note }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
import { ROLE_PERMISSION_ROWS, ROLES } from '@/constants/roles';
...
const roleCatalog = computed(() => {
    const notes = {
        [ROLES.SUPER_ADMIN]: 'Requires a linked personnel record.',
        [ROLES.ADMIN]: 'Requires a linked personnel record.',
        [ROLES.ENCODER]: 'Requires a linked personnel record. Same permissions as Guest.',
        [ROLES.GUEST]: 'Offered only when the account has no personnel record.',
    };

    const names = roles.value.length
        ? roles.value.map((role) => role.role_name)
        : [ROLES.ENCODER, ROLES.ADMIN, ROLES.SUPER_ADMIN, ROLES.GUEST];

    return names.map((role_name) => ({
        role_name,
        note: notes[role_name] ?? '',
    }));
});
...
async function loadRoles() {
    try {
        const options = await userService.fetchCreateOptions();
