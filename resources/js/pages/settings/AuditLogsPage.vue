        ...
        <div class="space-y-6">
            <div class="flex flex-wrap gap-2">
                <button
                    ...
                    :class="activeTab === 'audit' ? 'bg-brand text-white' : 'bg-white text-slate-600 ring-1 ring-slate-200 hover:bg-slate-50'"
                ...
                <button
                    type="button"
                    class="rounded-lg px-3 py-2 text-sm font-medium transition"
                    :class="activeTab === 'login' ? 'bg-brand text-white' : 'bg-white text-slate-600 ring-1 ring-slate-200 hover:bg-slate-50'"
            ...
            <form v-if="activeTab === 'audit'" class="rbim-card grid gap-3 p-4 sm:grid-cols-5" @submit.prevent="loadAuditLogs">
                <div>
            ...
            <div v-else class="rbim-card overflow-hidden">
                <div class="overflow-x-auto">
                    <table v-if="activeTab === 'audit'" class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <table v-else class="min-w-full divide-y divide-slate-200 text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr v-for="log in loginLogs" :key="log.user_log_id">
                                <td class="px-4 py-3 font-medium text-slate-900">{{ log.username || log.user_id }}</td>
...
async function loadLoginLogs() {
    ...
    try {
        loginLogs.value = await auditLogService.fetchLoginLogs();
    } catch (err) {
        error.value = extractErrorMessage(err, 'Unable to load login history.');
    } finally {
        loading.value = false;
    }
}
